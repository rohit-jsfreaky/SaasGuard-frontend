...
import type { Feature, OverrideType } from "@/types/entities";
import { CalendarIcon, X } from "lucide-react";
import { format } from "date-fns";
import { cn } from "@/lib/utils";

interface CreateOrgOverrideModalProps {
  features: Feature[];
}

export function CreateOrgOverrideModal({
  features,
}: CreateOrgOverrideModalProps) {
  const { isOpen, type, closeModal } = useOverridesModalStore();
  const createOrgOverride = useOverridesStore(
    (state) => state.createOrgOverride
  );
  ...
  const [errors, setErrors] = useState<{ feature?: string; value?: string }>(
    {}
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newErrors: { feature?: string; value?: string } = {};
    if (!featureSlug) {
    ...
    const success = await createOrgOverride({
      featureSlug,
      overrideType,
    ...
    <Dialog open={isModalOpen} onOpenChange={closeModal}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Create an override that applies to all members of this organization.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            >
              <div className="flex items-center space-x-2 rounded-md border p-3 hover:bg-accent cursor-pointer">
                ...
                <Label htmlFor="enable" className="flex-1 cursor-pointer">
              ...
              <div className="flex items-center space-x-2 rounded-md border p-3 hover:bg-accent cursor-pointer">
                ...
                <Label htmlFor="disable" className="flex-1 cursor-pointer">
          ...
          {overrideType === "limit_increase" && (
            <div className="grid gap-2">
              <Label htmlFor="value">New Limit Value</Label>
              <Input
                id="value"
                type="number"
                value={value}
          ...
          <div className="grid gap-2">
            ...
            <Textarea
              ...
              placeholder="e.g. Temporary access for beta testing"
              rows={2}
            />
          </div>
